  ...
  return (
    <div>
      ...
      <section className="p-6 border-b border-stone-200">
        <h2 className="text-2xl font-bold text-stone-800 mb-6">프로젝트 미리보기</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 w-fit mx-auto">
          {props.previewUrls.map((item, index) => (
            <img src={item} key={index} alt="" className="h-52 rounded-lg shadow object-cover" />
          ))}
        </div>
      </section>
      {/* 주요 기능 */}
      ...
      <section className="p-6">
        ...
        {/* 주요 기술 */}
        <div className="mb-8">
          <h3 className="text-xl font-semibold text-stone-800 mb-4">주요 기술</h3>
            ...
            {props.functions.map((item, index) => (
              <li key={index} className="p-4 bg-stone-100 rounded-lg">
                ...
                <div className="space-y-4">
                  <div>
                    <p className="text-stone-600 font-medium mb-2">선정 이유:</p>
        ...
        {/* 기타 사용 기술 */}
        <div>
          <h3 className="text-xl font-simibold text-stone-800 mb-4">기타 사용 기술</h3>
          <div className="bg-stone-100 rounded-lg p-4">
            <ul className="flex flex-col gap-4">
              {props.stackCategories.map((item, index) => (
                <li key={index}>
                  <h4 className="text-stone-700 font-medium mb-2">{item.title}</h4>
